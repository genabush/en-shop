...
export class CustomProductListComponentService {
  ...
  private searchResults$: Observable<ProductSearchPage> = this.productSearchService
    .getResults()
    .pipe(filter(searchResult => Object.keys(searchResult).length > 0));

  private searchByRouting$: Observable<ActivatedRouterStateSnapshot> = combineLatest([
   ...
   */
  readonly model$: Observable<ProductSearchPage> = combineLatest(this.searchResults$, this.searchByRouting$).pipe(
    pluck(0),
  ...
  private getCriteriaFromRoute(routeParams: ProductListRouteParams, queryParams: SearchCriteria): SearchCriteria {
    return {
  ...
  private getQueryFromRouteParams({ brandCode, categoryCode, query }: ProductListRouteParams) {
    if (query) {
      return query;
    }
    if (categoryCode) {
