...
import {
  HomepageService,
  HomepageType
} from "../pageDisplayConditions/HomepageService";
...
export class ManagePageService {
          ...
          .then(() => {
            ...
            return this.cmsitemsRestService.update(_pageInfo).then(() => {
              ...
              this.alertService.showSuccess({
                message:
                  "se.cms.actionitem.page.trash.alert.success.description",
                messagePlaceholders: {
                  pageName: pageInfo.name
                }
              });
    ...
    return this.confirmationModalService
      ...
      .then(() => {
        return this.cmsitemsRestService
          .delete(pageInfo.uuid)
          .then((response: void) => {
            this.alertService.showSuccess(
              "se.cms.page.permanently.delete.alert.success"
            );
  ...
  restorePage(pageInfo: ICMSPage): angular.IPromise<void> {
    ...
    return this.cmsitemsRestService.update(_pageInfo).then(
      (response: any) => {
        ...
        this.pageRestoredAlertService.displayPageRestoredSuccessAlert(
          _pageInfo
        );
      ...
      (result: any) => {
        ...
        this.waitDialogService.hideWaitModal();
        this.pageRestoreModalService.handleRestoreValidationErrors(
          _pageInfo,
          errors
        );
      }
  ...
  isPageTrashable(
    ...
    return this.$q
      .all([
        hasFallbackHomepageOrIsPrimaryWithoutVariationsPromise,
        isInWorkflowPromise
      ])
      .then(
        result => {
  ...
  getDisabledTrashTooltipMessage(
    ...
    return this.workflowService
      .isPageInWorkflow(pageInfo)
      .then(isPageInWorkflow => {
    ...
    this.pageInfoService.getPageUUID().then(
      (pageUUID: string) => {
        if (pageUUID) {
          ...
          this.$log.error(
            "deletePageService::deletePage - pageUUID is undefined"
          );
          deferred.reject();
        }
      },
      () => {
        if (pageInfo.activeLink) {
          deferred.resolve(
            "se.cms.actionitem.page.trash.activelink.confirmation.description.pagelist"
          );
        } else {
          deferred.resolve(
            "se.cms.actionitem.page.trash.confirmation.description.pagelist"
          );
        }
      }
    );
