...
module.exports = function(grunt) {
    ...
    return {
        ...
        config: function(data, conf) {
             ...
             */
            const config = {
                theBodyShopsmartedit: conf.unitSmartedit,
                theBodyShopsmarteditContainer: conf.unitSmarteditContainer,
                theBodyShopsmarteditcommons: {
                    options: {
            ...
            if (grunt.option('coverage')) {
                config.theBodyShopsmartedit.options = coverage(
                    customPaths.coverage.dir,
                    customPaths.coverage.smarteditDirName
                )(config.theBodyShopsmartedit.options);

                config.theBodyShopsmarteditcommons.options = coverage(
                    customPaths.coverage.dir,
                    customPaths.coverage.smarteditcommonsDirName
                )(config.theBodyShopsmarteditcommons.options);

                config.theBodyShopsmarteditContainer.options = coverage(
                    customPaths.coverage.dir,
                    customPaths.coverage.smarteditcontainerDirName
                )(config.theBodyShopsmarteditContainer.options);
            }
