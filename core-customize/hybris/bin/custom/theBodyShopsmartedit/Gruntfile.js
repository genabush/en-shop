...
module.exports = (grunt) => {
    ...
    grunt.registerTask('multiKarma', ['karma:theBodyShopsmartedit', 'karma:theBodyShopsmarteditcommons', 'karma:theBodyShopsmarteditContainer']);
    ...
    grunt.registerTask('test', 'run unit tests', function() {
        ...
        if (target && target.length) {
            target.forEach((t) => {
                if (t === 'inner') {
                    tasks.push('karma:theBodyShopsmartedit');
                } else if (t === 'outer') {
                    tasks.push('karma:theBodyShopsmarteditContainer');
                } else if (t === 'commons') {
                    tasks.push('karma:theBodyShopsmarteditcommons');
                }
            });
        } else {
            tasks = ['multiKarma'];
