 ...
 */
package uk.co.thebodyshop.fulfilmentprocess.strategy.impl;
...
public class SplitByWarehouse implements SplittingStrategy
{
	...
	protected List<WarehouseModel> getPossibleWarehouses(final AbstractOrderEntryModel orderEntry)
	{
		...
		if (orderEntry.getOrder().getStore() != null)
		{
			...
			candidateWarehouses.addAll(orderEntry.getOrder().getDeliveryPointOfService() == null
					? orderEntry.getOrder().getStore().getWarehouses() : orderEntry.getOrder().getDeliveryPointOfService().getWarehouses());
