...
import uk.co.thebodyshop.core.emailWhenInStock.ws.data.EmailWhenInStockWsDTO;
import uk.co.thebodyshop.core.emailWhenInStock.ws.data.EmailWhenInStockWsDTOForm;
import uk.co.thebodyshop.core.formatters.WsDateFormatter;
import uk.co.thebodyshop.core.product.data.ReviewDataList;
import uk.co.thebodyshop.core.product.data.SuggestionDataList;
...
import uk.co.thebodyshop.core.product.ws.data.ProductVisibilityWsDTO;
import uk.co.thebodyshop.core.queues.data.ProductExpressUpdateElementData;
import uk.co.thebodyshop.core.queues.data.ProductExpressUpdateElementDataList;
...
import uk.co.thebodyshop.core.v2.helper.ProductsHelper;
import uk.co.thebodyshop.core.validator.PointOfServiceValidator;
...
public class ProductsController extends BaseController
{
	...
	@Resource(name = "productVisibilityService")
	private ProductVisibilityService productVisibilityService;
	@Resource(name = "baseSiteService")
	private BaseSiteService baseSiteService;
	@Resource(name = "emailWhenInStockWsDTOFormValidator")
	private Validator emailWhenInStockWsDTOFormValidator;
	@Resource(name = "emailWhenInStockService")
	...
	@RequestMapping(value = "/visible/{productCode}", method = RequestMethod.GET)
	@CacheControl(directive = CacheControlDirective.PRIVATE, maxAge = 60)
	@Cacheable(value = "productVisibilityCache", key = "T(de.hybris.platform.commercewebservicescommons.cache.CommerceCacheKeyGenerator).generateKey(true,true,#productCode,#fields)")
	@ResponseBody
	@ApiOperation(nickname = "getProductVisibility", value = "Get product visibility.", notes = "Returns visibility information of a single product according to a product code.")
	@ApiBaseSiteIdParam
	public ProductVisibilityWsDTO getProductVisibility(@ApiParam(value = "Product identifier", required = true)
	@PathVariable
	...
	final String fields)
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("getProductVisibility: code=" + sanitize(productCode));
		}

		final ProductVisibilityData productVisibility = productVisibilityService.getVisibiltyInfo(productCode,
				activeProductCatalogVersionSupplier.get().orElse(null));
		return getDataMapper().map(productVisibility, ProductVisibilityWsDTO.class, fields);
	}
			...
			@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields, final HttpServletResponse response)
	{
		...
		final StoreFinderStockSearchPageData result = doSearchProductStockByLocation(productCode, location, latitude, longitude,
				currentPage, getBaseStoreService().getCurrentBaseStore().getMaxResultsForPosSearch());
	...
	@RequestMapping(value = "/createEmailWhenInStock", method = RequestMethod.POST, consumes = { MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE })
	@ResponseStatus(value = HttpStatus.CREATED)
	@ResponseBody
	@ApiOperation(nickname = "emailWhenInStock", value = " create emailWhenInStock", notes = "Create emailWhenInStock. Requires the following " + "parameters: email, productCode.")
	@ApiBaseSiteIdParam
	public EmailWhenInStockWsDTO emailWhenInStock(@ApiParam(value = "EmailWhenInStock object.", required = true)
	@RequestBody
	final EmailWhenInStockWsDTOForm form, @ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields, final HttpServletRequest httpRequest, final HttpServletResponse httpResponse)
	{
		validate(form, "form", emailWhenInStockWsDTOFormValidator);
		final EmailWhenInStockData data = new EmailWhenInStockData();
		data.setEmailId(form.getEmailId());
		data.setProductCode(form.getProductCode());
		final EmailWhenInStockResultData emailWhenInStockResultData = emailWhenInStockService.findEmailWhenInStock(data);
		return getDataMapper().map(emailWhenInStockResultData, EmailWhenInStockWsDTO.class, fields);
