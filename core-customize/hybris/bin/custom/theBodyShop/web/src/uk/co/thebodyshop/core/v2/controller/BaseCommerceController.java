...
import uk.co.thebodyshop.core.validator.PlaceOrderCartValidator;
...
public class BaseCommerceController extends BaseController
{
	...
	@Resource(name = "collectionAddressValidator")
	private Validator collectionAddressValidator;
	...
	/**
	 * Creates GeoPoint instance is lat, and lon are correct, else returns null
	 */
	protected GeoPoint ofGeoPoint(final Double lat, final Double lon)
	{
	...
	protected void updateCollectionAddressInternal(final AddressData collectionDeliveryAddress, final String firstName, final String lastName, final String cellPhone)
	{
		collectionDeliveryAddress.setFirstName(firstName);
		collectionDeliveryAddress.setCellphone(cellPhone);
		collectionDeliveryAddress.setLastName(lastName);
		getUserFacade().editAddress(collectionDeliveryAddress);
	}

	protected AddressData createCollectionAddressInternal(final AddressData addressData)
	{
		getUserFacade().addAddress(addressData);
		return addressData;
	}

	protected void updateCollectionAddressInternal(final AddressData collectionDeliveryAddress, final AddressData addressData)
	{
		addressData.setFirstName(collectionDeliveryAddress.getFirstName());
		addressData.setCellphone(collectionDeliveryAddress.getCellphone());
		addressData.setLastName(collectionDeliveryAddress.getLastName());
		addressData.setId(collectionDeliveryAddress.getId());
	...
	protected TbsCheckoutFacade getTbsCheckoutFacade()
	{
		return tbsCheckoutFacade;
	}

	protected Validator getCollectionAddressValidator()
	{
		return collectionAddressValidator;
	}
