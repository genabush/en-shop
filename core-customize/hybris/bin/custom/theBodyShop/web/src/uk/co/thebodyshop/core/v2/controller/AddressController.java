...
import uk.co.thebodyshop.core.swagger.ApiBaseSiteIdAndUserIdAndAddressParams;
import uk.co.thebodyshop.core.user.data.AddressDataList;
import uk.co.thebodyshop.core.validation.data.AddressValidationData;
...
public class AddressController extends BaseCommerceController
{
	...
	private static final String ADDRESS_MAPPING = "firstName,lastName,titleCode,phone,cellphone,line1,line2,town,postalCode,region(isocode),district,country(isocode),defaultAddress,companyName,visibleInAddressBook";
	...
	private TbsUserFacade tbsUserFacade;
	...
	public AddressListWsDTO getAddresses(@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields,@ApiParam(value = "The type of countries.", allowableValues = "SHIPPING,BILLING") @RequestParam(defaultValue ="SHIPPING" ) final String addressType)
	{
		if (StringUtils.isNotBlank(addressType) && !CountryType.SHIPPING.toString().equalsIgnoreCase(addressType) && !CountryType.BILLING
				.toString().equalsIgnoreCase(addressType))
		{
			throw new IllegalStateException(String.format("The value of country type : [%s] is invalid", addressType));
		}
		final List<AddressData> addressList = tbsUserFacade.getAddressBookByAddressType(addressType);
