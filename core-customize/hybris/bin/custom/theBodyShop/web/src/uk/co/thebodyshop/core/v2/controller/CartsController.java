...
import uk.co.thebodyshop.core.cart.impl.CommerceWebServicesCartFacade;
...
import uk.co.thebodyshop.core.deliveryrestrictions.error.ws.data.ProductDeliveryRestrictionErrorWsDTO;
import uk.co.thebodyshop.core.enums.FulfillmentMethodEnum;
import uk.co.thebodyshop.core.exceptions.InvalidPaymentInfoException;
import uk.co.thebodyshop.core.exceptions.NoCheckoutCartException;
import uk.co.thebodyshop.core.exceptions.UnsupportedDeliveryModeException;
...
import uk.co.thebodyshop.core.giftwrap.ws.dto.GiftWrapMessageRequestWSDTO;
import uk.co.thebodyshop.core.order.data.CartDataList;
import uk.co.thebodyshop.core.order.data.OrderEntryDataList;
import uk.co.thebodyshop.core.product.data.PromotionResultDataList;
import uk.co.thebodyshop.core.request.support.impl.PaymentProviderRequestSupportedStrategy;
import uk.co.thebodyshop.core.stock.CommerceStockFacade;
import uk.co.thebodyshop.core.voucher.data.VoucherDataList;
...
public class CartsController extends BaseCommerceController {
	...
	private TbsCouponFacade voucherFacade;
	@Resource(name = "tbsPaymentFacade")
	private TbsPaymentFacade tbsPaymentFacade;
	@Resource(name = "adyenCheckoutFacade")
	private TbsAdyenCheckoutFacade adyenCheckoutFacade;
	@Resource(name = "giftWrapFacade")
	private GiftWrapFacade giftWrapFacade;
	@Resource(name = "giftMessageValidator")
	private Validator giftMessageValidator;
	...
	protected static OrderEntryData getCartEntry(final CartData cart, final String productCode,
	...
	public CartWsDTO getCart(
			@ApiParam(value = "Response configuration. This is the list of fields that should be returned in the response body.", allowableValues = "BASIC, DEFAULT, FULL") @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields) {
		// CartMatchingFilter sets current cart based on cartId, so we can return cart
		// from the session
	...
	public CartWsDTO createCart(
			@ApiParam(value = "Anonymous cart GUID.") @RequestParam(required = false) final String oldCartId,
	...
	public AddressWsDTO createCartDeliveryAddress(final HttpServletRequest request,
		...
		getTbsCheckoutFacade().updateFulfillmentMethod(FulfillmentMethodEnum.DIRECT);
	...
	public AddressWsDTO createCartDeliveryAddress(
		...
		AddressData addressData = getDataMapper().map(address, AddressData.class,
				"titleCode,firstName,lastName,line1,line2,town,postalCode,country(isocode),region(isocode),defaultAddress,companyName,phone");
	...
	public void replaceCartDeliveryAddress(
			@ApiParam(value = "Address identifier", required = true) @RequestParam final String addressId)
					throws NoCheckoutCartException {
		if (null == getUserFacade().getAddressForCode(addressId))
		{
			return;
		}
		super.setCartDeliveryAddressInternal(addressId);
	...
	@Secured({ "ROLE_CUSTOMERGROUP", "ROLE_GUEST", "ROLE_CUSTOMERMANAGERGROUP", "ROLE_TRUSTED_CLIENT" })
	@RequestMapping(value = "/{cartId}/productdeliveryrestriction", method = RequestMethod.GET)
	@ResponseBody
	@ApiOperation(nickname = "getProductDeliveryRestrictionError", value = "Get product delivery restriction error.", notes = "Returns product names that are restricted to deliver for particular address")
	@ApiBaseSiteIdUserIdAndCartIdParam
	public ProductDeliveryRestrictionErrorWsDTO getProductDeliveryRestrictionError(
			@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("getProductDeliveryRestrictionError");
		}

		final ProductDeliveryRestrictionError productDeliveryRestrictionError = getTbsCheckoutFacade()
				.getProductDeliveryRestrictionError();
		return getDataMapper().map(productDeliveryRestrictionError, ProductDeliveryRestrictionErrorWsDTO.class, fields);
	}
	...
	public DeliveryModeListWsDTO getCartDeliveryModes(
		...
		final Map<DeliveryRestrictionError, List<? extends DeliveryModeData>> deliveryModeWithRestrictionMap = getTbsCheckoutFacade()
				.getSupportedDeliveryModeWithRestrictionMap();
		final DeliveryRestrictionError restrictionError = deliveryModeWithRestrictionMap.keySet().iterator().next();
		deliveryModesData.setDeliveryModes(deliveryModeWithRestrictionMap.get(restrictionError));
		deliveryModesData.setDeliveryModeRestrictionError(restrictionError);
	...
	public void doApplyCartVoucher(
		...
		if(voucherFacade.checkMaximumCouponAllowedForCart())
		{
			super.applyVoucherForCartInternal(voucherId);
		}
		else
		{
			throw new CouponServiceException("coupon.maximum.allowed.threshold");
		}
	}
	...
	@ApiBaseSiteIdUserIdAndCartIdParam
	public PaymentMethodsResponse getPaymentModes(
			@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields) {
		final PaymentMethodsResponse paymentMethods = tbsPaymentFacade.getSupportedPaymentMethods();
		try {
			paymentMethods.setSavedPayments(adyenCheckoutFacade.getPaymentDetails());
		} catch (final ApiException e) {
			LOG.error("API Exception", e);
		} catch (final Exception e) {
			LOG.error("Couldn't retrieve stored card details", e);
		}
		return paymentMethods;
	}

	@RequestMapping(value = "/{cartId}/reset", method = RequestMethod.POST)
	@Secured(
			{ "ROLE_CUSTOMERGROUP", "ROLE_TRUSTED_CLIENT", "ROLE_CUSTOMERMANAGERGROUP" })
	@ResponseBody
	@ApiOperation(nickname = "resetDeliveryDetails", value = "Reset delivery details of the cart", notes = "Reset the Delivery Details of the cart")
	@ApiBaseSiteIdUserIdAndCartIdParam
	public StatusWsDTO resetDeliveryDetails()
	{
		final StatusWsDTO status = new StatusWsDTO();
		if(getTbsCheckoutFacade().resetDeliveryDetails())
		{
			status.setCode("Success");
		}
		else
		{
			status.setCode("err.cart.not.found");
		}
		return status;
	}
	
	@RequestMapping(value = "/{cartId}/giftwrap/add", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(nickname = "addGiftWrap", value = "adds a gift wrap to cart.", notes = "adds a gift wrap to cart.")
	@ApiBaseSiteIdAndUserIdParam
	public StatusResponseWSDTO addGiftWrap(@ApiParam(value = "cartId", required = true) @PathVariable final String cartId, @ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		final StatusResponseData giftWrapStatusData = giftWrapFacade.addGiftWrapToCart();
		return getDataMapper().map(giftWrapStatusData, StatusResponseWSDTO.class, DEFAULT_FIELD_SET);
	}

	@RequestMapping(value = "/{cartId}/giftwrap/remove", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(nickname = "removeGiftWrap", value = "removes a gift wrap to cart.", notes = "removes a gift wrap to cart.")
	@ApiBaseSiteIdAndUserIdParam
	public StatusResponseWSDTO removeGiftWrap(@ApiParam(value = "cartId", required = true) @PathVariable final String cartId, @ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		final StatusResponseData giftWrapStatusData = giftWrapFacade.removeGiftWrapFromCart();
		return getDataMapper().map(giftWrapStatusData, StatusResponseWSDTO.class, DEFAULT_FIELD_SET);
	}
	
	@RequestMapping(value = "/{cartId}/giftwrap/message", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(nickname = "addGiftWrapMessage", value = "adds a gift wrap message to cart.", notes = "adds a gift wrap message to cart.")
	@ApiBaseSiteIdAndUserIdParam
	public StatusResponseWSDTO addGiftWrapMessage(@ApiParam(value = "cartId", required = true) @PathVariable final String cartId,
			@ApiParam(value = "The name and message of the gift wrap to add/update on cart model.", required = true) @RequestBody final GiftWrapMessageRequestWSDTO giftWrapMessageRequest,
			@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		if (StringUtils.isNotBlank(giftWrapMessageRequest.getGiftMessage()) || StringUtils.isNotBlank(giftWrapMessageRequest.getGiftMessageName()) || StringUtils.isNotBlank(giftWrapMessageRequest.getGiftMessageSenderName()))
		{
			validate(giftWrapMessageRequest, "giftWrapMessageRequest", giftMessageValidator);
		}
		final GiftWrapMessageData giftWrapMessageData = getDataMapper().map(giftWrapMessageRequest, GiftWrapMessageData.class);
		final StatusResponseData giftWrapStatusData = giftWrapFacade.addGiftMessageToCart(giftWrapMessageData);
		return getDataMapper().map(giftWrapStatusData, StatusResponseWSDTO.class, DEFAULT_FIELD_SET);
	}
