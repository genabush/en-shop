...
public class CollectionPointDeliveryModesStrategyTest
{
	...
	@InjectMocks
	private CollectionPointDeliveryModesStrategy collectionPointDeliveryModesStrategy;
	...
	@Test
	public void tesCollectionPointOrder()
	{
		when(order.getFulfillmentMethod()).thenReturn(FulfillmentMethodEnum.COLLECTION);
		collectionPointDeliveryModesStrategy.filter(deliveryModeList, order);
		Assert.assertTrue(deliveryModeList.size() == 1);
		Assert.assertTrue(deliveryModeList.iterator().next().getCode().equalsIgnoreCase(COLLECTIONPOINT_DELIVERY_MODE));
	}
