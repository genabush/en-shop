 ...
 * @author Marcin
 */
public class BaseProductVariantsValueResolver extends AbstractValueResolver<TbsBaseProductModel, Object, Object>
{
	private static final Logger LOG = Logger.getLogger(BaseProductVariantsValueResolver.class);

	private final ProductVisibilityService productVisibilityService;

	private final UserPriceGroupService userPriceGroupService;

	public BaseProductVariantsValueResolver(final ProductVisibilityService productVisibilityService, final UserPriceGroupService userPriceGroupService)
	{
		...
		if (baseSiteModel != null && baseSiteModel.getStores() != null && !baseSiteModel.getStores().isEmpty())
		{
			...
			if (Objects.nonNull(baseStore))
			{
				if (CollectionUtils.isNotEmpty(product.getVariants()))
				{
					...
					for (final VariantProductModel variantModel : visibleVariants)
					{
						document.addField(indexedProperty, variantModel.getCode(), resolverContext.getFieldQualifier());
					}
				}
			}
		}
