 ...
 * @author prateek.goel
 */
public class CollectionPointDeliveryModesStrategy implements DeliveryModeRestrictionStrategy
{
	...
	public boolean filter(final List<DeliveryModeModel> deliveryModes, final AbstractOrderModel order)
	{
		...
		if (CollectionUtils.isNotEmpty(deliveryModes) && FulfillmentMethodEnum.COLLECTION == fulfillmentMethod)
		{
			final List<DeliveryModeModel> restrictedDeliveryModes = new ArrayList<>();

			for (final DeliveryModeModel deliveryMode : deliveryModes)
			{
				if (BooleanUtils.isFalse(deliveryMode.getCollectionPoint()) || BooleanUtils.isTrue(deliveryMode.getForCollectInStore()))
				{
